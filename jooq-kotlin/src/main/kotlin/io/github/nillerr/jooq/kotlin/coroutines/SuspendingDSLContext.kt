...
class SuspendingDSLContext(internal val dsl: DSLContext) {
    ...
    suspend fun <R> transaction(
        ...
        transactional: suspend CoroutineScope.(Configuration) -> R,
    ): R {
     ...
     * database
     */
    suspend fun <R : UpdatableRecord<R>> updateAll(records: List<R>): Int {
        val queries = records
            .filter { it.changed() }
            .map { record ->
                dsl.updateQuery(record.table).also {
                    it.setRecord(record)
        ...
        val updates = dsl.batch(queries).suspend().execute().sum()
     ...
     */
    suspend fun <R : UpdatableRecord<R>> deleteAll(records: List<R>): Int {
     ...
     */
    suspend fun <R : TableRecord<R>> insertAll(records: List<R>): Int {
        val record = records.firstOrNull()
        if (record == null) {
            return 0
        }
