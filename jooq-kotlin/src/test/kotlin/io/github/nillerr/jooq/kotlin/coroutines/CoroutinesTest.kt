...
class CoroutinesTest {
    ...
    fun awaitTransaction(): Unit = runBlocking {
        ...
        dsl.suspend().transaction { trx ->
            trx.dsl().suspend().insert(lisa)
            trx.dsl().assertExists(lisa)
            trx.dsl().suspend().delete(lisa)
        }
    ...
    fun `awaitTransaction unwraps DataAccessException`(): Unit = runBlocking {
        ...
        val result = runCatching {
            dsl.suspend().transaction { trx ->
                trx.dsl().suspend().insert(lisa1)
                trx.dsl().suspend().insert(lisa2)
            }
        }
    ...
    fun `awaitInsert without changes`() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().suspend().insert(user)

        // Then
        assertEquals(0, result)
        trx.dsl().assertNotExists(user)
    }
    ...
    fun `awaitInsertAll without values`() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().suspend().insertAll(users)
    ...
    fun `awaitInsertAll without changes`() = dsl.runTest { trx ->
        ...
        // Then
        assertEquals(0, result)
        trx.dsl().assertNotExists(john)
        trx.dsl().assertNotExists(jane)
    }
    ...
    fun `awaitInsertAll with changes`() = dsl.runTest { trx ->
        ...
        trx.dsl().assertExists(john)
        trx.dsl().assertExists(jane)
    }
    ...
    fun `awaitInsertOnConflictDoNothing without changes`() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().suspend().insertOnConflictDoNothing(user)
    ...
    fun `awaitInsertOnConflictDoNothing when user exists`() = dsl.runTest { trx ->
        // Given
        val user = Users.john
        trx.dsl().suspend().insert(Users.john)
    ...
    fun `awaitInsert throws IntegrityConstraintViolationException when user exists`() = dsl.runTest { trx ->
        ...
        // When
        val result = runCatching { trx.dsl().suspend().insert(Users.john) }
    ...
    fun `awaitUpdate without changes`() = dsl.runTest { trx ->
        // Given
        val user = Users.john
        trx.dsl().suspend().insert(user)

        // When
        val result = trx.dsl().suspend().update(user)
    ...
    fun `awaitDelete when user exists`() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().suspend().delete(user)
    ...
    fun `awaitDeleteAll when no users are provided`() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().suspend().deleteAll(users)
    ...
    fun `awaitDeleteAll when users exist`() = dsl.runTest { trx ->
        // Given
        val john = Users.john
        trx.dsl().suspend().insert(john)

        val jane = Users.jane
        trx.dsl().suspend().insert(jane)
    ...
    fun `awaitStore when user does not exist`() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().suspend().store(user)
    ...
    fun `awaitToMap with key selector and value selector`() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().selectFrom(USER)
    ...
    fun awaitToMap() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().select(USER.USERNAME, DSL.count())
            .from(USER)
    ...
    fun awaitValueToList() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().select(USER.ID)
            .from(USER)
            .suspend().valueToList()
    ...
    fun `awaitSingle when multiple users match`() = dsl.runTest { trx ->
        ...
        // When
        val result = runCatching { trx.dsl().selectFrom(USER).suspend().single() }
    ...
    fun `awaitCount when user not exist`() = dsl.runTest { trx ->
        ...
        // When
        val result = trx.dsl().suspend().count(USER, DSL.trueCondition(), USER.ID)
    ...
    companion object {
        ...
        fun beforeAll() {
            ...
            val configuration = PooledJDBCCoroutineDispatcherConfiguration(dsl).copy(poolSize = 10)
            dsl.jdbcCoroutineDispatcher = PooledJDBCCoroutineDispatcher(configuration)
        }
    ...
    fun `awaitTransaction - JDBC - concurrent`(): Unit = runBlocking {
        ...
        // When
        withTimeout(30_000) {
            nettyWorkerSimulationDispatcher().use { nettyDispatcher ->
                Executors.newScheduledThreadPool(nThreads).asCoroutineDispatcher().use { dispatcher ->
                    withContext(dispatcher) {
                        repeat(nThreads) { requester ->
                            launch {
                                repeat(20) { iteration ->
                                    withContext(nettyDispatcher) {
                                        ...
                                        val trxThread = dsl.suspend().transaction { trx ->
                                            ...
                                            val record = newUserRecord(iteration)
                                            trx.dsl().suspend().insert(record)

                                            assertContinuationOnThread(trxThread)
                                            simulateTransactionalDispatchedAPICall(trxThread)
                                            assertContinuationOnThread(trxThread)

                                            trx.dsl().assertExists(record)

                                            assertContinuationOnThread(trxThread)
                                            simulateTransactionalDispatchedAPICall(trxThread)
                                            assertContinuationOnThread(trxThread)

                                            trx.dsl().suspend().delete(record)
...
//                                    delay(random.nextLong(200))
        ...
        println("[CoroutinesTest] `awaitTransaction - JDBC - concurrent` completed in $duration")

//        val serialThreadSelectionIndicatorLimit = 3
//
//        val groups = observer.observations.entries.groupBy({ (_, obseorvation) -> observation.count }, { (thread, _) -> thread })
//        assert(groups.size > serialThreadSelectionIndicatorLimit) {
//            groups.forEach { (observations, threads) ->
//                println("[Observation] $observations on each of ${threads.joinToString(", ") { it.name }}")
//            }
//            "Thread usage indicates serial thread selection"
//        }
